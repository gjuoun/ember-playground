...
module('Integration | Component | map', function (hooks) {
  ...
  test('it renders a map image for the specified parameters', async function (assert) {
    ...
    // get src attribute from an element
    let {src} = find('.map img');
    let token = encodeURIComponent(ENV.mapbox.accessToken);
  ...
  test('it updates the `src` attribute wen the arguments change', async function (assert) {
    // `this` here does not refer to the component instance
    this.setProperties({
      lat: 37.7749,
      lng: -122.4194,
      zoom: 10,
